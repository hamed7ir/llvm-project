...
; Exercise NonSemantic DebugLine emission driven by per-instruction DebugLoc.
;
; Reference: SPIRV-LLVM-Translator LLVMToSPIRVDbgTran::transLocationInfo, which
; emits, for each translated instruction whose (file, line, column) differs from
; the previous one:
;
...
;
; The whole-function CHECK block below shows both the semantic instructions
; (OpIAdd / OpIMul / OpReturnValue) and the interleaved non-semantic ones
; (DebugFunctionDefinition, DebugLine) in the exact order they appear.
...
; Line and column operands are OpConstant i32. Each value is emitted once and
; reused (dedup), so constants that repeat across statements share one id:
;   V3 = line 3 = column-start of line 5;  V4 = column-end of line 5;
;   V5 = line 5;  V10/V11 = columns of line 3;  V12/V13 = columns of line 4.
; Trailing {{$}} anchors avoid a prefix like "3" matching "32".
